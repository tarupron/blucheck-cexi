...
function ui.get_spells()
    ...
    for x = 0, 2048 do
        ...
        if (spell ~= nil and spell.Skill == 43 and spell.LevelRequired[16 + 1] > 0 and spell.LevelRequired[16 + 1] <= 75) then
            ui.spells:append(T{
                index   = x,
                name    = spell.Name[1],
        ...
        if (res == nil) then
            ...
            if spell.zones:len() > 0 then
                ...
                table.sort(zoneKeys, function(a, b)
                    if tonumber(a) == currentZoneId then
                        return true
                    elseif tonumber(b) == currentZoneId then
                        return false
                    else
